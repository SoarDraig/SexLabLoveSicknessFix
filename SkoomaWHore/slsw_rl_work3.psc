...
;Debug.Notification("Hooker3 Client!")
int Roll = (scum.RanInt + 2)
if roll == redlight.random
	;Debug.Notification("Work3: Failed to find client")
	setStage(50)
	return
endif
Client.Clear()
	...
	If (Whore3.getRef() as Actor).getDistance(WRPosition) <= 1500
		(Client.getRef() as Actor).MoveTo(WRMarker)
		SetStage(20)
	elseif (Whore3.getRef() as Actor).getDistance(WHPosition) <= 1500
		(Client.getRef() as Actor).MoveTo(WHMarker)
		SetStage(20)
	elseif (Whore3.getRef() as Actor).getDistance(SOPosition) <= 1500
		(Client.getRef() as Actor).MoveTo(SOMarker)
		SetStage(20)
	elseif (Whore3.getRef() as Actor).getDistance(RIPosition) <= 1500
		(Client.getRef() as Actor).MoveTo(RIMarker)
		SetStage(20)
	elseif (Whore3.getRef() as Actor).getDistance(MKPosition) <= 1500
...
RegisterForModEvent("AnimationEnd_Whore3", "Whore3Client")

Int Random = Utility.RandomInt(1, 6)

actor[] sexActors = new actor[2]
sexActors[0] = Whore3.getRef() as Actor
...
SexLab.StartSex(sexActors, anims, allowBed=true, hook="Whore3")
...
UnregisterforModEvent("AnimationEnd_Whore3")
