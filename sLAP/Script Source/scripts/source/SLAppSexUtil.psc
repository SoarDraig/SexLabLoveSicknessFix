...
Function StartSexSmall(Actor src, Actor dst, bool rape = false, bool PostScene, String EventRegisterDummy, String EventName)
...
;	Debug.notification(dst + "StartSexActors")
	...
	sexActors[0] = dst
	sexActors[1] = src
	self._startSexActorsShotaHook(sexActors, rape, PostScene, EventRegisterDummy, EventName)
EndFunction
...
Function _startSexActorsShotaHook(Actor[] actors, bool Rape, bool Hook, String EventRegisterDummy, String EventName)
	;Debug.notification(target + "_startSexActors")
	Actor victim
	actors = SexLab.SortActors(actors)
	sslBaseAnimation[] anims
	;anims = self._buildAnimation(actors, caller, Rape)
	anims = SexLab.GetAnimationsByTags(actors.Length, "Shota", "")
...
Function BaboDialogueRandomintSet(Int Chance, Bool Worse)
	BaboDialogueRandomint = Chance
	WorseBool = Worse
EndFunction
...
Event BaboDialogueAfter(string eventName, string argString, float argNum, form sender)
	Actor Victim = SexLab.HookActors(argString)[0]
	Actor Rapist = SexLab.HookActors(argString)[1]
	(SLApproach_Main as SLApproachBaseQuestScript).BaboDialogueSLAPPTrigger(Rapist, WorseBool, BaboDialogueRandomint)
EndEvent

Event ToxicFoodRapeAfter(string eventName, string argString, float argNum, form sender)
	Actor Victim = SexLab.HookActors(argString)[0]
	Actor Rapist = SexLab.HookActors(argString)[1]
	(SLApproachAskForSexQuest as SLAppPCSexQuestScript).SimpleForcereference(Rapist)
	SLAppDruggedFoodSceneAfter.ForceStart()
	(SLApproach_Main as SLApproachBaseQuestScript).BaboDialogueSLAPPTrigger(Rapist, WorseBool, BaboDialogueRandomint)
EndEvent


Scene Property SLAppAfterProstitution Auto
Scene Property SLAppDruggedFoodSceneAfter Auto
...
Int Property BaboDialogueRandomint Auto Hidden
Bool Property WorseBool Auto Hidden
